/*
 * $Id: MySqlUserQueries.java,v 1.3 2005/06/07 12:32:35 bel70 Exp $
 *
 * ***** BEGIN LICENSE BLOCK *****
 ...
 */
class MySqlUserQueries extends UserQueries {
	...
	private static final String GET_USER_INFO = "SELECT user_signature, user_name, user_city, user_status, count(jrf_message.id) as tot_mes"
			+ " FROM jrf_user, jrf_message WHERE user_name =?  AND user_name = jrf_message.sender GROUP BY user_name";

	private static final String GET_USERS = "SELECT user_name, id, user_status FROM jrf_user ORDER BY user_name LIMIT ?,?"; // block,
																															// user_toshow

	private final static String SET_LAST_INTIME = "UPDATE jrf_user SET last_intime=NOW() WHERE user_name = ?";
