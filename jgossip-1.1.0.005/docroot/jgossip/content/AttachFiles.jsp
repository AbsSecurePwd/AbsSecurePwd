...
--%> <%@ include file="../jspf/jsp_header.jspf"%>
 <gossip:ifconfig key="<%=IConst.CONFIG.ENABLE_FILE_UPLOAD%>">
		<c:set var="jrf_AttachFiles" value="Y"/>
  </gossip:ifconfig>
  <c:if test="${empty jrf_AttachFiles}">
...
<%@ include file="../jspf/messagePreview.jspf"%>   
<script>
function rewriteAction(formObj){
	try{
		...
		for(var i in formObj.elements){
			...
			if(fieldObj.value&&(fieldObj.type=="text"||fieldObj.type=="hidden"||fieldObj.type=="textarea")){
				url=appendParameter(url,fieldObj.name, fieldObj.value);
			}
		}
		formObj.action=url;
	}catch(error){
	 	alert('Error caught by rewriteAction()\ntype=' + error.type + '\nname=' +error.name);
	 	throw error;
	}
}
function appendParameter(url, parameter, value){
  	var delimeter = url.indexOf('?') == -1 ? '?' : '&';
  	return url + delimeter + escape(parameter) + '=' + escape(value);
}
</script> 	
<html:form method="post" action="/AddMessage" onsubmit="rewriteAction(this);hideErrors();return validateAddMessageForm(this);" enctype="multipart/form-data">
