...
public class ForumHibernateDAO implements ForumDAOInterface {
	...
	public void createCategory(ForumCategory cat) throws DAOException {
		...
		try {
			...
			List results;
			results = session.find("select max(forumCategory.componentPosition) from " + ForumCategory.class.getName()
					+ " forumCategory ");
	 ...
	 * 
	 * @see
	 * net.sf.jboard.model.forum.dao.ForumDAOInterface#createForumItem(net.sf.
	 * jboard.model.forum.dto.ForumItem)
	 */
	 ...
	 * 
	 * @see
	 * net.sf.jboard.model.forum.dao.ForumDAOInterface#deleteForumItem(long)
	 */
	 ...
	 * 
	 * @see net.sf.jboard.model.forum.dao.ForumDAOInterface#createForumThread(net.sf.jboard.model.forum.dto.ForumThread,
	 *      net.sf.jboard.model.forum.dto.ForumMessage)
	 */
	...
	public ForumRoot readPanorama() throws DAOException {
		...
		try {
			...
			session = ForumHibernateUtil.currentSession();
			List forumCategories = session.find(
					"from net.sf.jboard.model.forum.dto.ForumCategory forumcategory order by forumcategory.componentPosition ");
	...
	private Long getComponentPosition(Class componentClass, Long parentComponentId) throws HibernateException {
		List results;
		results = session.find("select max(forumComponent.componentPosition) from " + componentClass.getName()
				+ " forumComponent where fk_parent_id = ?", parentComponentId, Hibernate.LONG);
		Long max = (Long) results.get(0);
		if (max != null) {
			max = new Long(max.longValue() + 1);
		} else {
