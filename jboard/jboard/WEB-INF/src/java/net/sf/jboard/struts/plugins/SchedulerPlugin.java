...
/**
 * Plugin lancé au démarrage de l'applicatiàon par la servlet de Struts. ce
 * plugin est dédié au lancement des jobs du scheduler.
 * 
 * @author cga.
 *
 */
public class SchedulerPlugin implements PlugIn {
	...
	public void init(ActionServlet arg0, ModuleConfig arg1) throws ServletException {
		...
		try {
			...
			JobDetail jobDetail = new JobDetail("SampleJob", Scheduler.DEFAULT_GROUP, SampleJob.class);
			// définition du trigger qui prend en charge le job
			trigger = new CronTrigger("SampleTrigger", Scheduler.DEFAULT_GROUP, "SampleJob", Scheduler.DEFAULT_GROUP,
					cronExpression, TimeZone.getTimeZone("Europe/Paris"));

			// les timeZone peuvent aussi être exprimés avec un code à 3 lettres
			// pour "Europe/Paris" c'est ECT
			// on aurait pu aussi définir le temps en fontion de GMT ± des
			// heures
