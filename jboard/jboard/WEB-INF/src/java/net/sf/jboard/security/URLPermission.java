...
public class URLPermission extends java.security.Permission {
	...
	public String getActions() {
		// we do not have any actions int this class;just allow or deny acces to
		// URL
		return "";
	}

	public int hashCode() {
		return this.getName().hashCode(); // if we have any actions it should
											// be: getName().hashCode() ^
											// acion_mask
	}
	...
	public class InnerClass extends java.security.PermissionCollection {
		...
		public void add(java.security.Permission permission) {
			...
			if (permissions.get("*") == null) {
				...
				// if permission without wildcard, look first for wildcarded if
				// not found - add else skip
				java.util.Iterator iter = permissions.keySet().iterator();
				while (iter.hasNext()) {
					String key = (String) iter.next();
					if (key.endsWith("*")) {
		...
		public java.util.Enumeration elements() {
			// must use Hashtable becouse of old Collection framework used in
			// this API
