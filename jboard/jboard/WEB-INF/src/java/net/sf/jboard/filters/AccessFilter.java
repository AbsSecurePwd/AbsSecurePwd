...
public class AccessFilter implements Filter, Constants {
	...
	/**
	 * the method which handle the request. this method was inspired by the
	 * article on jaas published at
	 * <a href="http://www.mooreds.com/jaas.html">this</a> address.
	 * 
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (subject == null || req.getRequestURI().equals("/jboard/reg/LogonProcess.do")) {
			...
			if (!authenticationResult) {
				...
				res.sendRedirect(
						"/jboard/SwitchToModule.do?prefix=/reg&page=/Logon.do&authenticationFailed=authentication%20Failed");
			} else {
				sess = req.getSession();
		...
		/*
		 * if(AuthUtils.permitted(subject,permission)){ //redirect to loginpage
		 * because user is not authorized //to access to this resource
		 * res.sendRedirect(logonPage); }else{ // user is authorized
		 * chain.doFilter(request,response); }
		 */
