...
public class WeakAuthenticationCookie extends LessonAdapter
{
	...
	protected String checkParams(WebSession s, String password)
		throws Exception
	{
		//String p = password;
		String username = s.getParser().getStringParameter(USERNAME, "");
	
		WeakAuthenticationCookie w = new WeakAuthenticationCookie();
		...
		if (username.length() > 0 && password.length() > 0)
		{
			...
			{
				// make a new cookie
				String mangleLoginString = mangleLogin(username, password);
				Cookie newCookie = new Cookie(AUTHCOOKIE, mangleLoginString);
