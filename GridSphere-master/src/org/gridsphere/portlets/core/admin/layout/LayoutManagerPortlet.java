...
public class LayoutManagerPortlet extends ActionPortlet {
	...
	public static final String HELP_JSP = "admin/layout/help.jsp";
	...
	public void doSaveTab(ActionFormEvent event) throws PortletException, IOException {
		...
		if (comp instanceof PortletTab) {
			...
			if (!req.getLocale().getLanguage().equals("en")) {
				// inserted by Bastian Boegel, University of Ulm, 2009
				// set default language if not already set, otherwise empty tabs are 
				// shown for other languages
				String title_en = tab.getTitle("en");
				if ((title_en == null) || (title_en.length() == 0))
